#ifndef SEQAN_HEADER_SHAPE_QGRAM2_H
#define SEQAN_HEADER_SHAPE_QGRAM2_H
...
class Shape<TString, GappedShape2>
{

public:

	//q = volle qgramlänge inkl lücken
	int q;
	int num_gaps;
	int /*const*/ term;
	//shape_len ist die länge des shape vektors
	int shape_len;
	//[num_beachten num_gaps num_beachten num_gaps ... num_beachten]
	String<int> shape;
...
template <typename TString>
void
setShape(Shape<TString,GappedShape2> & me,int num_gaps, int q, int alp_size, int shape_len)
{
...
template <typename TString>
inline int &
numGaps(Shape<TString, GappedShape2>  & me)
{
	return me.num_gaps;
}
...
template <typename TString>
inline int &
shapeLen(Shape<TString, GappedShape2>  & me)
{
	return me.shape_len;
}
...
template <typename TString>
void
stringToShape(String<char> & shape_string,Shape<TString,GappedShape2> & shape)
{
	int count_gaps = 0;
	int shape_len = 1;
	...
	while(string_it < string_end) 
	{
		if(*string_it == '_')
			++count_gaps;
		if(*string_it_last != *string_it)
			++shape_len;
		++string_it_last;
		++string_it;
	}
	setShape(shape,count_gaps,length(shape_string),ValueSize<Shape<TString,GappedShape2> >::VALUE,shape_len);
