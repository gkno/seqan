...
/*
	...
	TStringSet& str = stringSet(g);
	typedef String<TSize> TColumnList;
	typedef String<TColumnList> TSeqColumn;
	TSeqColumn posPerSequence;
	fill(posPerSequence, alignSeqMem, String<TSize>() );
	typedef typename Iterator<TSegmentString>::Type TSegIter;
	TSegIter itSeg = begin(alignSeq);
	TSegIter itSegEnd = end(alignSeq);
	TSize col = 0;
	for(;itSeg != itSegEnd; goNext(itSeg)) {
		typedef typename Iterator<TVertexString>::Type TVertexIter;
		TVertexIter itVertex = begin(value(itSeg));
		TSize lengthCol = fragmentLength(g, value(itVertex));
		TVertexIter itVertexEnd = end(value(itSeg));
		for(;itVertex != itVertexEnd; goNext(itVertex)) {
			appendValue(value(posPerSequence, idToPosition(str, sequenceId(g, value(itVertex)))), col);
			appendValue(value(posPerSequence, idToPosition(str, sequenceId(g, value(itVertex)))), (col + lengthCol));
		}
		col += lengthCol;
	}

	// Walk through the profiles and check for disrupted sequences
	typedef std::set<TSize> TRealignSet;
	TRealignSet realignReads;
	typedef typename Iterator<TSeqColumn>::Type TSeqColIter;
	TSeqColIter itSeqCol = begin(posPerSequence);
	TSeqColIter itSeqColEnd = end(posPerSequence);
	TSize seqI = 0;
	for(;itSeqCol != itSeqColEnd; goNext(itSeqCol), ++seqI) {
		typedef typename Iterator<TColumnList>::Type TColumnIter;
		TColumnIter itCol = begin(value(itSeqCol));
		TColumnIter itColEnd = end(value(itSeqCol));
		if (itCol == itColEnd) continue;
		goNext(itCol);
		TSize oldCol = value(itCol);
		goNext(itCol);
		while(itCol != itColEnd) {
			if (value(itCol) - oldCol > 5) realignReads.insert(positionToId(str, seqI));
			goNext(itCol);
			oldCol = value(itCol);
			goNext(itCol);
		}
	}
	std::cout << "Unaligned reads: " << realignReads.size() << std::endl;

	// Eliminate these disrupted reads from the profiles
	TSegmentString tmpAlignSeq;
	itSeg = begin(alignSeq);
	itSegEnd = end(alignSeq);
	for(;itSeg != itSegEnd; goNext(itSeg)) {
		TVertexString tmpVertexStr;
		typedef typename Iterator<TVertexString>::Type TVertexIter;
		TVertexIter itVertex = begin(value(itSeg));
		TVertexIter itVertexEnd = end(value(itSeg));
		for(;itVertex != itVertexEnd; goNext(itVertex)) {
			if (realignReads.find(sequenceId(g, value(itVertex))) == realignReads.end()) {
				appendValue(tmpVertexStr, value(itVertex));
			}
		}
		if (!empty(tmpVertexStr)) appendValue(tmpAlignSeq, tmpVertexStr);
	}

	// Realign reads
	TSize seqMem = alignSeqMem - realignReads.size();
	for(typename TRealignSet::const_iterator seqIt = realignReads.begin(); seqIt != realignReads.end(); ++seqIt) {
		itSeg = begin(alignSeq);
		itSegEnd = end(alignSeq);
		for(;itSeg != itSegEnd; goNext(itSeg)) clear(value(itSeg));
		clear(alignSeq);
		TSegmentString seq1;
		_buildLeafString(g, *seqIt, seq1);
		heaviestCommonSubsequence(g,seq1,(TSize) 1,tmpAlignSeq,seqMem,alignSeq);
		++seqMem;
		itSeg = begin(tmpAlignSeq);
		itSegEnd = end(tmpAlignSeq);
		for(;itSeg != itSegEnd; goNext(itSeg)) clear(value(itSeg));
		clear(tmpAlignSeq);
		itSeg = begin(alignSeq);
		itSegEnd = end(alignSeq);
		for(;itSeg != itSegEnd; goNext(itSeg)) {
			TVertexString tmpVertexStr = value(itSeg);
			appendValue(tmpAlignSeq, tmpVertexStr);
		}
	}
*/		
