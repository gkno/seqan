...
// TODO(holtgrew): Suffixing parameter names with an underscore is not very nice.
// TODO(holtgrew): Maybe put the Gotoh algorithms into _affine.h and the NW algorithms into _linear.h.
// TODO(holtgrew): Often, the template parameter names are not chosen well.

//////////////////////////////////////////////////////////////////////////////
//needleman wunsch alignment
template <typename TScoreValue, typename TSeedConfig, unsigned DIMENSION, typename TString, typename TValue, typename TSpecSeed>
TScoreValue
						 ...
						 Score<TScoreValue, Simple> const & scoringScheme)
{
	...
	TScoreValue scoringSchemegap = scoreGapExtend(scoringScheme);

	TScoreValue horizontalValue = 0;
	TScoreValue border_ = scoringSchemegap;
	...
	border_ = down_height * scoringSchemegap;
	...
	*finger2 = inf;
	...
	for (int i = -1; i != down_height; ++i){	
		...
		for (unsigned int j = 0; j != run_length; ++j){
			if (*x==*y){
				*finger2 = verticalValue + scoringSchemematch;
			} else {
				tmp = *finger1;
				TScoreValue s1 = verticalValue + scoringSchememismatch;
				TScoreValue s2 = scoringSchemegap + ((horizontalValue > tmp) ? horizontalValue : tmp);
				*finger2 = (s1 > s2) ? s1 : s2;
			}
        ...
        Score<TScoreValue, Simple> const & scoringScheme)
{
	...
	TScoreValue scoringSchemediff = scoreMismatch(scoringScheme) - scoreGapExtend(scoringScheme);
	...
	while ((it_0 != it_0_end) && (it_1 != it_1_end))
	{
		...
		else
		{
			...
			gv = (v >= h) | (d + scoringSchemediff >= h);
			gh = (h >  v) | (d + scoringSchemediff >= v);
		}
		if (gv){
			++it_1;
			goNext(source_, 1);
...
template <typename TSource, typename TSpec, typename TScoreValue, typename TValue, typename TSpecSeed, typename TSeedConfig>
TScoreValue
bandedAlignment(
        ...
        NeedlemanWunsch const &)
{
	...
	TMatrix matr;
	TScoreValue ret = _banded_needleman_wunsch(matr, seed, k, sourceSegment(row(align_, 0)), sourceSegment(row(align_, 1)), scoringScheme);

    // TODO(holtgrew): Why is the matrix not given in the traceback algorithm directly?
	Iter<TMatrix, PositionIterator> iter = begin(matr);
	setPosition(iter, 1 + k + getLowerDiagonal(seed) - getStartDiagonal(seed));
	_banded_needleman_wunsch_trace(align_, iter, scoringScheme);
        ...
        Gotoh const &)
{
	...
	TMatrix v_matr;
	TMatrix h_matr;
	TScoreValue ret = _banded_gotoh(d_matr, v_matr, h_matr, seed, k, sourceSegment(row(align_, 0)), sourceSegment(row(align_, 1)), scoringScheme);
	_banded_gotoh_trace(align_, d_matr, v_matr, h_matr, 1 + k + getLowerDiagonal(seed) - getStartDiagonal(seed));	
	return ret;
}
        ...
        Score<TScoreValue, Simple> const & scoringScheme)
{
	...
	if (scoreGapOpen(scoringScheme) == scoreGapExtend(scoringScheme))
		return bandedAlignment(align_, seed, k, scoringScheme, NeedlemanWunsch());
	else
		return bandedAlignment(align_, seed, k, scoringScheme, Gotoh());
}
