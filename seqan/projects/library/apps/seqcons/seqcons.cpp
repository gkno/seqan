...
void __AllAgainstAll(AlignConfig<TTop, TLeft, TRight, TBottom> ac) {
	...
	for(TSize i = 0; i < 10000; ++i) {
		...
		TGraph g1(str);
		int sc1 = globalAlignment(g1, score_type, ac, NeedlemanWunsch());
		std::cout << sc1 << std::endl;
		std::cout << g1 << std::endl;
		TGraph g2(str);
		int sc2 = globalAlignment(g2, score_type, ac, lowDiag, highDiag, BandedNeedlemanWunsch());
		std::cout << lowDiag << ',' << highDiag << std::endl;
		std::cout << sc2 << std::endl;
		std::cout << g2 << std::endl;
		TGraph g3(str);
		int sc3 = globalAlignment(g3, score_type, ac, -1 * length(dna2), length(dna1), BandedNeedlemanWunsch());
		...
		if ((sc1 != sc2) || (sc2 != sc3)) {
			...
			std::cerr << "Scores: :" << matchScore << ',' << misMatchScore << ',' << gapScore << std::endl;
			exit(0);
		}
	}
}
