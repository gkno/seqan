...
struct IndelSimOptions
{
	...
	CharString outputInfo;	// and one for indel information
	CharString inputInfo;	// and one for indel information
...
//		fileName << options.outputInfo;
	if (*toCString(options.outputInfo) != 0)
		fileName << options.outputInfo;
	else
	{
		//if (*options.output != 0)
	...
	TOptions				&options)
{
	...
	while (!_streamEOF(file))
	{
		...
		_parse_skipWhitespace(file, c);
		indel.indelSize = _parse_readNumber(file,c) - indel.originalPos;
		indel.duplication = 0;
					  ...
					  TOptions &options)
{
	...
	if (readGFF(toCString(options.inputInfo), indelSet, genomes, gIdStringToIdNumMap, options) > 0) 
	{
		std::cerr << "Template indels " << options.inputInfo << " can't be loaded." << std::endl;
		return 0;
	}
	...
	TOptions 			&options)		// options
{
	...
	if(empty(options.ranges) && *toCString(options.inputInfo) == 0)
	{
	...
	bool result = true;
	if(*toCString(options.inputInfo) != 0)
	...
	while(it != indelMap.end())
	{
		...
		if(chromosome != lastChromosome) // reset to local position 0 if we are on a new chromosome
		{
			...
			append(simGenomes[lastChromosome],infix(genomes[lastChromosome],lastOriPos,length(genomes[lastChromosome])));
		...
		append(simGenomes[chromosome],infix(genomes[chromosome],lastOriPos,chromosomalPos));
		...
		else	// insertion
		{
			if(duplication)	// duplicate sequence ending at current position
				append(simGenomes[chromosome],infix(genomes[chromosome],chromosomalPos+indelSize,chromosomalPos));
	...
	if(lastOriPos != length(genomes[lastChromosome]))
		append(simGenomes[lastChromosome],infix(genomes[lastChromosome],lastOriPos,length(genomes[lastChromosome])));
	for(unsigned i = 0; i < length(genomes); ++i)
		if(empty(simGenomes[i])) simGenomes[i] = genomes[i];
