        ...
        for (unsigned j = 0; j < length(haplotypeIds); ++j) {
            ...
            for (unsigned k = 0; k < length(instructions); ++k) {
                ...
                if (options.generateMatePairs) {
                    ...
                    sprintf(readName, "%s.%09u/%d contig=%s haploid=%u length=%lu orig_begin=%u orig_end=%u edit_string=", outFileName, readId / 2, mateNum, toCString(fragmentStore.contigNameStore[inst.contigId]), haplotypeId, length(read), origBeginPos, origEndPos);
                } else {
                    sprintf(readName, "%s.%09u contig=%s haploid=%u length=%lu orig_begin=%u orig_end=%u edit_string=", outFileName, readId, toCString(fragmentStore.contigNameStore[inst.contigId]), haplotypeId, length(read), origBeginPos, origEndPos);
                }
                for (unsigned i = 0; i < length(inst.editString); ++i) {
                    char buffer[2] = "*";
