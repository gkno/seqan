...
///Two DNA sequences that shall be aligned.
	...
	TSequence seq1 = "NNNNNNACGACTNNNACTACTNNNNNN";
	TSequence seq2 = "ACTACTACT";
...
///a suitable algorithm (@Tag.Global Alignment Algorithms|Gotoh@) is automatically choosen.
	...
	SVGFile svgTrace("/tmp/pairwise1.svg");	
	_globalAlignment(svgTrace, stringSet(align), score, AlignConfig<true,false,false,false>(), NeedlemanWunsch());

	reverseComplementInPlace(seq1);
	reverseComplementInPlace(seq2);

	assignSource(row(align, 0), seq1);
	assignSource(row(align, 1), seq2);

	SVGFile svgTraceRC("/tmp/pairwise1_rc.svg");	
	_globalAlignment(svgTraceRC, stringSet(align), score, AlignConfig<true,false,false,false>(), NeedlemanWunsch());
...
#if 0
	SVGFile svg("/tmp/small_case.svg");
