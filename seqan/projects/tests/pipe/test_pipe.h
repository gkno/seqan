...
//        srand( (unsigned)time( NULL ) );
	...
	bool testVector(unsigned maxSize = 16*1024*1024) {
		...
		for(unsigned i = 1; i <= maxSize; i = i << 1) {
            ...
            for(unsigned *cur = buf.begin; cur != buf.end; ++cur) {
                if (*cur != *I) {
                    ::std::cout << ::std::endl << "testVector failed at position " << (cur - buf.begin) << " ";
                    return false;
                }
				++I;
			}
    ...
    bool testContainers(unsigned maxSize = 32*1024*1024) {
		bool result = true;
        time_t start, pool, mapper, sorter;
        time(&start);

//____________________________________________________________________________
		::std::cout << "Testing External Vector ... " << ::std::endl;
		if (testVector(maxSize) || (result = false)) {
			::std::cout << "OK.";
		} else
			::std::cout << "FAILED.";
		::std::cout << ::std::endl;
...
//____________________________________________________________________________
        ...
        time(&sorter);
		::std::cout << " " << (sorter - mapper) << " seconds" << ::std::endl;
